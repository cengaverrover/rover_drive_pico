...
void createMicroRosTask() {
  ...
  xTaskCreateAffinitySet(task::microRosTask, microRosTaskName.data(),
                         microRosTaskStack, nullptr, microRosTaskPriority,
                         microRosTaskCoreAffinity,
                         &task::microRosTaskHandle);
}
...
void createMotorTasks() {
  ...
  constexpr etl::array<etl::string_view, 4> taskNames{
      "motor_task_0", "motor_task_1", "motor_task_2", "motor_task_3"};
  ...
  for (int i = 0; i < taskNames.size(); i++) {
    xTaskCreateAffinitySet(task::motorTask, taskNames[i].data(), motorTaskStack,
                           indexes[i], motorTaskPriority, motorTaskCoreAffinity,
                           &task::motorTaskHandles[i]);
  }
}
} // namespace freertos
