// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from rover_mobility_interfaces:msg/MotorFeedback.idl
// generated code does not contain a copyright notice
#include "rover_mobility_interfaces/msg/detail/motor_feedback__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
rover_mobility_interfaces__msg__MotorFeedback__init(rover_mobility_interfaces__msg__MotorFeedback * msg)
{
  if (!msg) {
    return false;
  }
  // dutycycle
  // rpm
  // current
  return true;
}

void
rover_mobility_interfaces__msg__MotorFeedback__fini(rover_mobility_interfaces__msg__MotorFeedback * msg)
{
  if (!msg) {
    return;
  }
  // dutycycle
  // rpm
  // current
}

bool
rover_mobility_interfaces__msg__MotorFeedback__are_equal(const rover_mobility_interfaces__msg__MotorFeedback * lhs, const rover_mobility_interfaces__msg__MotorFeedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // dutycycle
  if (lhs->dutycycle != rhs->dutycycle) {
    return false;
  }
  // rpm
  if (lhs->rpm != rhs->rpm) {
    return false;
  }
  // current
  if (lhs->current != rhs->current) {
    return false;
  }
  return true;
}

bool
rover_mobility_interfaces__msg__MotorFeedback__copy(
  const rover_mobility_interfaces__msg__MotorFeedback * input,
  rover_mobility_interfaces__msg__MotorFeedback * output)
{
  if (!input || !output) {
    return false;
  }
  // dutycycle
  output->dutycycle = input->dutycycle;
  // rpm
  output->rpm = input->rpm;
  // current
  output->current = input->current;
  return true;
}

rover_mobility_interfaces__msg__MotorFeedback *
rover_mobility_interfaces__msg__MotorFeedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rover_mobility_interfaces__msg__MotorFeedback * msg = (rover_mobility_interfaces__msg__MotorFeedback *)allocator.allocate(sizeof(rover_mobility_interfaces__msg__MotorFeedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rover_mobility_interfaces__msg__MotorFeedback));
  bool success = rover_mobility_interfaces__msg__MotorFeedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rover_mobility_interfaces__msg__MotorFeedback__destroy(rover_mobility_interfaces__msg__MotorFeedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rover_mobility_interfaces__msg__MotorFeedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rover_mobility_interfaces__msg__MotorFeedback__Sequence__init(rover_mobility_interfaces__msg__MotorFeedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rover_mobility_interfaces__msg__MotorFeedback * data = NULL;

  if (size) {
    data = (rover_mobility_interfaces__msg__MotorFeedback *)allocator.zero_allocate(size, sizeof(rover_mobility_interfaces__msg__MotorFeedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rover_mobility_interfaces__msg__MotorFeedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rover_mobility_interfaces__msg__MotorFeedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rover_mobility_interfaces__msg__MotorFeedback__Sequence__fini(rover_mobility_interfaces__msg__MotorFeedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rover_mobility_interfaces__msg__MotorFeedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rover_mobility_interfaces__msg__MotorFeedback__Sequence *
rover_mobility_interfaces__msg__MotorFeedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rover_mobility_interfaces__msg__MotorFeedback__Sequence * array = (rover_mobility_interfaces__msg__MotorFeedback__Sequence *)allocator.allocate(sizeof(rover_mobility_interfaces__msg__MotorFeedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rover_mobility_interfaces__msg__MotorFeedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rover_mobility_interfaces__msg__MotorFeedback__Sequence__destroy(rover_mobility_interfaces__msg__MotorFeedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rover_mobility_interfaces__msg__MotorFeedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rover_mobility_interfaces__msg__MotorFeedback__Sequence__are_equal(const rover_mobility_interfaces__msg__MotorFeedback__Sequence * lhs, const rover_mobility_interfaces__msg__MotorFeedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rover_mobility_interfaces__msg__MotorFeedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rover_mobility_interfaces__msg__MotorFeedback__Sequence__copy(
  const rover_mobility_interfaces__msg__MotorFeedback__Sequence * input,
  rover_mobility_interfaces__msg__MotorFeedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rover_mobility_interfaces__msg__MotorFeedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rover_mobility_interfaces__msg__MotorFeedback * data =
      (rover_mobility_interfaces__msg__MotorFeedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rover_mobility_interfaces__msg__MotorFeedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rover_mobility_interfaces__msg__MotorFeedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rover_mobility_interfaces__msg__MotorFeedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
